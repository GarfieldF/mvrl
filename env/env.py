...
class CarlaEnv1():
    def __init__(self, index, bottom_policy, command_net):
        ...
        rospy.Subscriber('/scout/map', Image, self.mapCallback)
    ...
    def mapCallback(self, data):
        ...
        try:
            ...
            print(e)
        self.Map_image = np.asarray(image_data, dtype=np.float32) * np.float32(1.0 / 255.0)
    ...
    def get_command(self):
        out = self.command_net.predict([self.Map_image[None]])
        command = np.argmax(out[0])
        if command == 0:
            self.command = [1,0,0,0]
            print('***************************')
            print('follow lane')
    ...
    def control_vel(self, action):
        ...
        move_cmd.angular.z = action[1]*0.4     # 0.3
